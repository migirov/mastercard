    ...
    try {
      switch (event?.eventType) {
        case 'STATUS_CHG':
          ...
          // ВАЖНО при реализации: дедуп-ключ `wh:${ref}` ставится ВЫШЕ, до записи.
          // del-on-failure (ниже) спасает от ошибки в процессе, но НЕ от жёсткого
          // краша между set(dedup) и commit(статус): ключ останется, MC-ретрай уйдёт
          // в 'duplicate', событие потеряется. Когда появится БД-запись статуса —
          // ставить дедуп-ключ В ТОЙ ЖЕ транзакции, что и запись (атомарно).
          break;
        default:
