...
async function main(): Promise<void> {
  ...
  try {
    ...
    check(
      'GET /crossborder/balances (sandbox)',
      bal.status === 200,
      `HTTP ${bal.status}`,
    );
    ...
    check(
      'POST /crossborder/quotes (sandbox, passthrough)',
      qOk,
      `HTTP ${q.status}`,
    );
    ...
    const qb = await http.post('/crossborder/quotes', badAmount, {
      headers: internal,
    });
    check(
      'POST /crossborder/quotes с amount=number → 400',
      qb.status === 400,
      `HTTP ${qb.status}`,
    );
    ...
    check(
      'POST /admin/tenants OWN без secretRef → 400',
      tn.status === 400,
      `HTTP ${tn.status}`,
    );
    ...
    check(
      'POST /oauth/token grant_type=password → 400',
      tok.status === 400,
      `HTTP ${tok.status}`,
    );
    ...
    check(
      'POST /webhooks/mastercard без токена → 401',
      wh.status === 401,
      `HTTP ${wh.status}`,
    );
    ...
    check(
      'POST /webhooks/mastercard с токеном → 200',
      wh2.status === 200,
      `HTTP ${wh2.status}`,
    );

    // 8) SafeIdPipe: пустой ?ref= → 400 (валидация на границе, не в сервисе)
    const refEmpty = await http.get('/crossborder/payments?ref=', {
      headers: internal,
    });
    check(
      'GET /crossborder/payments?ref= (пусто) → 400',
      refEmpty.status === 400,
      `HTTP ${refEmpty.status}`,
    );

    // 9) SafeIdPipe: ref со слэшем → 400 (анти path-injection)
    const refSlash = await http.get('/crossborder/payments?ref=a%2Fb', {
      headers: internal,
    });
    check(
      'GET /crossborder/payments?ref=a/b → 400',
      refSlash.status === 400,
