// ISO-3166-1 countries: alpha-2 (what an IBAN starts with) and alpha-3 (what Mastercard
// expects). Generated, not hand-typed -- the English names come from Intl.DisplayNames, so
// the only hand-maintained data is the alpha-2/alpha-3 pairing.
//
// Used by the beneficiary-address check, which needs an explicit country: the API previously
// defaulted to USA in silence, so a Tel Aviv address was validated against US address rules
// and came back "invalid" with nobody able to see why.

/** @typedef {{ alpha2: string, alpha3: string, name: string }} Country */
...
export const COUNTRIES = [
  ...
  { alpha2: 'AG', alpha3: 'ATG', name: "Antigua & Barbuda" },
  ...
  { alpha2: 'BA', alpha3: 'BIH', name: "Bosnia & Herzegovina" },
  ...
  { alpha2: 'CG', alpha3: 'COG', name: "Congo - Brazzaville" },
  { alpha2: 'CD', alpha3: 'COD', name: "Congo - Kinshasa" },
  ...
  { alpha2: 'HM', alpha3: 'HMD', name: "Heard & McDonald Islands" },
  { alpha2: 'HN', alpha3: 'HND', name: "Honduras" },
  { alpha2: 'HK', alpha3: 'HKG', name: "Hong Kong SAR China" },
  ...
  { alpha2: 'MO', alpha3: 'MAC', name: "Macao SAR China" },
  ...
  { alpha2: 'MM', alpha3: 'MMR', name: "Myanmar (Burma)" },
  ...
  { alpha2: 'PS', alpha3: 'PSE', name: "Palestinian Territories" },
  ...
  { alpha2: 'ST', alpha3: 'STP', name: "São Tomé & Príncipe" },
  ...
  { alpha2: 'GS', alpha3: 'SGS', name: "South Georgia & South Sandwich Islands" },
  ...
  { alpha2: 'BL', alpha3: 'BLM', name: "St. Barthélemy" },
  { alpha2: 'SH', alpha3: 'SHN', name: "St. Helena" },
  { alpha2: 'KN', alpha3: 'KNA', name: "St. Kitts & Nevis" },
  { alpha2: 'LC', alpha3: 'LCA', name: "St. Lucia" },
  { alpha2: 'MF', alpha3: 'MAF', name: "St. Martin" },
  { alpha2: 'PM', alpha3: 'SPM', name: "St. Pierre & Miquelon" },
  { alpha2: 'VC', alpha3: 'VCT', name: "St. Vincent & Grenadines" },
  { alpha2: 'SD', alpha3: 'SDN', name: "Sudan" },
  { alpha2: 'SR', alpha3: 'SUR', name: "Suriname" },
  { alpha2: 'SJ', alpha3: 'SJM', name: "Svalbard & Jan Mayen" },
  ...
  { alpha2: 'WF', alpha3: 'WLF', name: "Wallis & Futuna" },
...
/** alpha-2 (e.g. from an IBAN prefix) -> alpha-3, or '' when unknown. */
export function alpha2ToAlpha3(alpha2) {
  const hit = COUNTRIES.find((c) => c.alpha2 === String(alpha2 || '').toUpperCase());
  return hit ? hit.alpha3 : '';
}

/**
 * Best-guess country for a beneficiary account. An IBAN always starts with the ISO-3166
 * alpha-2 of the account's country, which makes it a good DEFAULT -- but only a default:
 * it is shown in an editable field so the operator can correct it, which is the part that
 * was missing when the country was chosen silently.
 */
export function countryFromAccount(account) {
  const s = String(account || '').replace(/\s+/g, '').toUpperCase();
  return /^[A-Z]{2}[0-9]{2}/.test(s) ? alpha2ToAlpha3(s.slice(0, 2)) : '';
}

/** alpha-3 -> display name, falling back to the code itself. */
export function countryName(alpha3) {
  const hit = COUNTRIES.find((c) => c.alpha3 === alpha3);
  return hit ? hit.name : String(alpha3 || '');
}
