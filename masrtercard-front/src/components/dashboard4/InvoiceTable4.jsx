...
import InvoiceDetailDrawer from '@/components/dashboard/InvoiceDetailDrawer';
import InvoiceDocViewer from '@/components/payment/chatbot-tabs/InvoiceDocViewer';
import { AnimatePresence } from 'framer-motion';

const currencySymbols = { ILS: '₪', USD: '$', EUR: '€' };

export default function InvoiceTable4({ invoices, selectedIds, onToggleSelect, onToggleAll, isKybVerified, onPaySingle, onViewStatus }) {
  ...
  return (
    <>
                ...
                return (
                  <TableRow
                    ...
                    className={`cursor-pointer transition-colors hover:bg-muted/40 ${isSelected ? 'bg-primary/5' : ''} ${
                      isKybVerified && isCrossBorder && isPayable ? 'ring-1 ring-inset ring-rose-200' : ''
                    }`}
                  >
                    ...
                    <TableCell>
                      <div className="flex items-center gap-2">
                        ...
                        {invoice.file_url && (
                          <button onClick={(e) => { e.stopPropagation(); setDocInvoice(invoice); }}
                            className="p-0.5 rounded hover:bg-primary/10 text-primary transition-colors" title="View document">
                    ...
                    <TableCell onClick={(e) => e.stopPropagation()}>
                      <button onClick={(e) => { e.stopPropagation(); onViewStatus(invoice); }} className="hover:opacity-80 transition-opacity">
                        <InvoiceStatusBadge status={invoice.status} />
                      </button>
                    </TableCell>
                    <TableCell className="text-right" onClick={(e) => e.stopPropagation()}>
                      {isPayable ? (
                        <Button size="sm" disabled={!isKybVerified} onClick={() => onPaySingle(invoice)}
                          className={`text-xs h-8 px-3 ${isKybVerified ? 'bg-rose-600 hover:bg-rose-700 text-white' : 'bg-muted text-muted-foreground cursor-not-allowed'}`}>
                          Pay now
                        </Button>
                      ) : (
      ...
      <AnimatePresence>
        {selectedInvoice && (
          <InvoiceDetailDrawer invoice={selectedInvoice} onClose={() => setSelectedInvoice(null)} onPay={onPaySingle} isKybVerified={isKybVerified} />
        )}
      </AnimatePresence>

      {docInvoice && (
        <InvoiceDocViewer fileUrl={docInvoice.file_url} invoiceNumber={docInvoice.invoice_number} onClose={() => setDocInvoice(null)} />
      )}
    </>
  );
}