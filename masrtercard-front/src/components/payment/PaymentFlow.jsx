...
export default function PaymentFlow({ invoices, onClose, onComplete, profile }) {
  ...
  const stepMessages = {
    review: 'Please review and verify each invoice below. I\'ve enriched some fields via Mastercard APIs — highlighted in blue.',
    batch_overview: 'Here\'s your final payment summary. Review each item and confirm.',
    funding: 'Let me check your account balances...',
    completion: 'Your payments are being processed!',
  };
    ...
    switch (STEPS[currentStep]) {
        ...
        return (
          <InvoiceReviewStep
            ...
            onProceed={() => {
              ...
              addMessage({ role: 'assistant', content: stepMessages.batch_overview });
              goNext();
            }}
          />
        );
        ...
        return (
          <BatchOverviewStep
            ...
            onProceed={() => {
              ...
              addMessage({ role: 'assistant', content: stepMessages.funding });
              goNext();
            }}
          />
        );
      case 'funding':
        return (
          <FundingStep
            ...
            profile={profile}
            onProceed={() => {
              addMessage({ role: 'assistant', content: stepMessages.completion });
              goNext();
            }}
          />
        );
      case 'completion':
        return (
          <CompletionStep
            invoices={reviewedInvoices}
            onDone={() => onComplete(reviewedInvoices)}
          />
        );
      default:
        return null;
        ...
        <div className="flex items-center gap-2 py-2">
          {STEPS.map((step, i) => (
            <div key={step} className="flex items-center gap-2">
              ...
              {i < STEPS.length - 1 && (
                <div className={`w-8 h-0.5 ${i < currentStep ? 'bg-primary' : 'bg-border'}`} />
              )}
            </div>
          ))}
        </div>
