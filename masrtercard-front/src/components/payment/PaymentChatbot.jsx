...
import PaymentsTab from './chatbot-tabs/PaymentsTab';
import ReportsTab from './chatbot-tabs/ReportsTab';
import SettingsTab from './chatbot-tabs/SettingsTab';
import KnowledgeBaseTab from './chatbot-tabs/KnowledgeBaseTab';
import AIChatTab from './chatbot-tabs/AIChatTab';
import { BankOnboardingInline } from '@/components/onboarding/BankOnboarding';
...
// viewMode: 'chat' (narrow side panel) | 'half' (50% width) | 'full' (full screen)
const VIEW_WIDTHS = {
...
function WelcomeState({ profile, onSelectInvoices }) {
  const balances = [
...
export default function PaymentChatbot({ invoices, onClose, onComplete, profile, defaultTab = 'pay', onOnboardingComplete }) {
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      case 'pay':
        if (!onboardingDone) {
          return (
            <BankOnboardingInline onComplete={() => {
        ...
        return invoices?.length > 0 ? (
          <PaymentFlow invoices={invoices} onClose={onClose} onComplete={onComplete} profile={profile} />
        ) : (
          <WelcomeState profile={profile} />
        );
      case 'payments':
        return <PaymentsTab />;
      case 'reports':
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-border shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-lg bg-primary flex items-center justify-center">
              <span className="text-primary-foreground text-[10px] font-bold leading-tight text-center">XBS</span>
            </div>
            <h3 className="text-sm font-bold">Embedded XBS</h3>
          </div>
          <div className="flex items-center gap-1">
            {/* Chat mode */}
            <Button
              variant="ghost" size="icon" className={`h-8 w-8 ${viewMode === 'chat' ? 'text-primary bg-primary/10' : ''}`}
            ...
            {/* Half screen */}
            <Button
              variant="ghost" size="icon" className={`h-8 w-8 ${viewMode === 'half' ? 'text-primary bg-primary/10' : ''}`}
            ...
            {/* Full screen */}
            <Button
              variant="ghost" size="icon" className={`h-8 w-8 ${viewMode === 'full' ? 'text-primary bg-primary/10' : ''}`}
          ...
          {TABS.map((tab) => {
            ...
            return (
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-1.5 px-1 py-2.5 text-xs font-medium transition-all border-b-2 ${
                  isActive
