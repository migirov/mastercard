...
export default function InvoiceTable({ invoices, selectedIds, onToggleSelect, onToggleAll, isKybVerified, onPaySingle, onViewStatus }) {
  const [selectedInvoice, setSelectedInvoice] = useState(null);
  const [docInvoice, setDocInvoice] = useState(null);
  const payableInvoices = invoices.filter(inv => 
    ['unpaid', 'rfi'].includes(inv.status) &&
    !['Anonymous Partner', 'Bank X – TopUp'].includes(inv.supplier_name)
  );
  ...
  return (
          ...
          <TableHeader>
            <TableRow className="bg-muted/50">
              <TableHead className="w-12">
                <Checkbox
                  checked={allSelected}
                  onCheckedChange={() => onToggleAll(payableInvoices)}
                ...
                >
                  <TableCell onClick={(e) => e.stopPropagation()}>
                    <Checkbox
                      ...
                      disabled={!isKybVerified || !isPayable}
                    />
                  </TableCell>
                  <TableCell>
                    <div className="flex items-center gap-2">
                      ...
                      {invoice.file_url && (
                        <button
                          onClick={(e) => { e.stopPropagation(); setDocInvoice(invoice); }}
                  ...
                  <TableCell>
                    <button onClick={() => onViewStatus(invoice)} className="hover:opacity-80 transition-opacity">
    ...
    {docInvoice && (
      <InvoiceDocViewer
        fileUrl={docInvoice.file_url}
        invoiceNumber={docInvoice.invoice_number}
