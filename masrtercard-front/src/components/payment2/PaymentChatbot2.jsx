...
import PaymentFlow2 from './PaymentFlow2';
...
import { BankOnboardingInline2 } from '@/components/onboarding/BankOnboarding2';
...
function WelcomeState2({ profile }) {
  const balances = [
...
export default function PaymentChatbot2({ invoices, onClose, onComplete, profile, defaultTab = 'pay', onOnboardingComplete }) {
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      case 'pay':
        if (!onboardingDone) {
          return (
            <BankOnboardingInline2 onComplete={() => {
        ...
        return invoices?.length > 0 ? (
          <PaymentFlow2 invoices={invoices} onClose={onClose} onComplete={onComplete} profile={profile} />
        ) : (
          <WelcomeState2 profile={profile} />
        );
      case 'payments':
        return <PaymentsTab entityName="Invoice2" />;
      case 'reports':
        return <ReportsTab entityName="Invoice2" />;
      case 'knowledge':
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-border shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-lg bg-indigo-600 flex items-center justify-center">
              <span className="text-white text-[10px] font-bold leading-tight text-center">XBS</span>
            </div>
            <h3 className="text-sm font-bold">Embedded XBS <span className="text-indigo-500 font-normal text-xs ml-1">Partner 2</span></h3>
          </div>
          <div className="flex items-center gap-1">
            <Button variant="ghost" size="icon" className={`h-8 w-8 ${viewMode === 'chat' ? 'text-indigo-600 bg-indigo-50' : ''}`} onClick={() => setViewMode('chat')} title="Chat panel">
              <PanelRight className="w-4 h-4" />
            </Button>
            <Button variant="ghost" size="icon" className={`h-8 w-8 ${viewMode === 'half' ? 'text-indigo-600 bg-indigo-50' : ''}`} onClick={() => setViewMode('half')} title="Half screen">
              <Minimize2 className="w-4 h-4" />
            </Button>
            <Button variant="ghost" size="icon" className={`h-8 w-8 ${viewMode === 'full' ? 'text-indigo-600 bg-indigo-50' : ''}`} onClick={() => setViewMode('full')} title="Full screen">
