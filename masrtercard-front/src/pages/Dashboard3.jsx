...
import InvoiceTable3 from '@/components/dashboard3/InvoiceTable3';
import ActivationBanner3 from '@/components/onboarding/ActivationBanner3';
import BankOnboarding3 from '@/components/onboarding/BankOnboarding3';
import ExistingPaymentAccountModal from '@/components/onboarding/ExistingPaymentAccountModal';
import PaymentChatbot3 from '@/components/payment3/PaymentChatbot3';
...
export default function Dashboard3() {
  ...
  const { data: invoices = [], isLoading: loadingInvoices } = useQuery({
    queryKey: ['invoices3'],
    queryFn: () => api.entities.Invoice3.list('-created_date'),
  });
  ...
  const updateProfile = useMutation({
    mutationFn: ({ id, data }) => api.entities.CompanyProfile3.update(id, data),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['company_profile3'] }),
  });

  const updateInvoices = useMutation({
    mutationFn: async (updates) => {
      for (const { id, data } of updates) {
        await api.entities.Invoice3.update(id, data);
      }
    },
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['invoices3'] }),
  });
  ...
  const handleViewInvoice = (invoice) => {
    ...
    setChatbotDefaultTab(isPayable ? 'pay' : 'payments');
    setShowChatbot(true);
  };
  ...
  const simulateStatusProgression = async (invoicesList) => {
    ...
    setTimeout(async () => {
      for (const id of ids) {
        await api.entities.Invoice3.update(id, { status: 'processing' });
      }
      ...
      setTimeout(async () => {
        for (const inv of invoicesList) {
          ...
          await api.entities.Invoice3.update(inv.id, {
  ...
  return (
    <div className="max-w-[1400px] mx-auto pb-24 md:pb-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <div className="flex items-center gap-3 mb-1">
            <div className="w-9 h-9 rounded-xl bg-white border border-border overflow-hidden flex items-center justify-center p-0.5 shadow-sm">
              <img
          ...
          <p className="text-sm text-muted-foreground">Manage and process your invoice payments via Altshuler Shaham</p>
        </div>
        <Button
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <Button variant="outline" onClick={() => queryClient.invalidateQueries({ queryKey: ['invoices3'] })}>
      ...
      {loadingInvoices ? (
        ...
        <InvoiceTable3
          ...
          onViewStatus={handleViewInvoice}
        />
      )}

      {/* Modals */}
      <AnimatePresence>
        {showBanner && <ActivationBanner3 onJoinNow={handleJoinNow} onDismiss={() => setShowBanner(false)} onHaveAccount={handleHaveAccount} />}
      </AnimatePresence>

      <AnimatePresence>
        {showOnboarding && <BankOnboarding3 onComplete={handleOnboardingComplete} onClose={() => setShowOnboarding(false)} />}
      </AnimatePresence>
      ...
      {!showChatbot && (
        <motion.button
          ...
          title="Open XBS Partner 3"
        >
          <span className="font-black text-base leading-none tracking-tight">XBS</span>
          <span className="text-[9px] text-primary-foreground/70 font-medium">P3</span>
        </motion.button>
      )}
