...
- ✅ **Валидация ENV** — `ConfigModule.forRoot({ validate })` (Zod),
...
- ✅ Unit (jest): **20 сьютов / 147 тестов** — зелёные.
...
2. 🟡 **Аутентичность вебхука = mTLS (бывший «C1»):** по официальной доке MC
   ...
   есть подписи payload для проверки в коде нет, поэтому каркас noop-verifier-а удалён
   в issue #7. Сейчас активный фактор — fail-closed `X-Webhook-Token`.
