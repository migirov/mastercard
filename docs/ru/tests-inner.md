...
| **`credentials.*`** (разбит, issue #14) | `own-credentials.cache`: дедуп stampede (одновременные resolve → один fetch), потолок LRU ≤500 (старейший вытеснен), rejected fetch выселяется (не залипает на TTL), re-resolve по TTL, invalidate. `own-credentials.provider`: allowlist `partnerId` + анти-traversal `secretRef` (`..`) + валидация бандла + контракт 422. `platform-credentials.provider`: сборка + кэш-parse-once + warm в onModuleInit. `credentials.service`: dispatch фасада PLATFORM/OWN + invalidate + throw на неизвестный режим |
