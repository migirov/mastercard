...
- **Ретеншн БД (открытый item, инфра).** После issue #4 KV-слой с TTL убран — идемпотентность
  платежей (`payment_idempotency`) и дедуп/персист вебхуков (`tx_status`) ведутся в Postgres
  **без app-level TTL** (cron-очистки больше нет, по требованию тимлида). Следствия: (1) обе
...
- **Провижен тенантов при встраивании (issue #5).** Встраиваемый `MastercardModule` больше НЕ
...
- ~~Расширить Swagger-аннотации~~ — **сделано** в рамках code-quality review.

---

## История аудита (исправлено)

Проведён **10-цикловый аудит** (баги / безопасность / оптимизация) + **2 цикла
регрессии** (каждый цикл = аудит→фикс, всё верифицировано). **Открытых HIGH/MED
нет.** Затем code-quality review по 4 перспективам (архитектура / поддерживаемость /
API-контракт / тестирование) → внедрены «Tier 1»-рефакторинги (централизованная
карта путей MC; композитный декоратор `UseGatewayContract()`; barrel `src/index.ts`;
закрыты Swagger-gaps; +4 регресс-спека). Тесты: 20 сьютов / 147 unit-тестов + 23/23
e2e против живого sandbox (на живом Postgres).

Ранний 4-цикловый аудит (2026-06-10), все правки прошли typecheck:
