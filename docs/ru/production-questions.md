...
- [ ] **Декрипт зашифрованного push** (MTF/Prod). `WebhookHandler` детектирует
  ...
  но не расшифровывает. Доделать: вызвать `decryptResponse` в обработчике (per-tenant ключи
  уже строятся `EncryptionService`-ом — см. «Решено»). Затык: в push нет атрибуции тенанта
  (partnerId под шифром) → для PLATFORM расшифровка платформенным ключом, для OWN нужен
  роутинг ключа. На sandbox push «Not Applicable» — кейс проверяется только на MTF/Prod.
