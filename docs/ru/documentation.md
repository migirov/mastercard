...
| кэш креды | **cache-manager (memory, TTL+LRU) per-pod** | кэш | источник истины — Vault/env; поды кэшируют независимо из одного источника, TTL ограничивает staleness |
...
(Vault/KMS), а не в нашей БД. Из бандла `OwnCredentialsProvider` собирает
...
- **Зашифрованный push** (`encrypted_payload.data`): сырой конверт **ПЕРСИСТИТСЯ** в
  `tx_status` с `eventType='ENCRYPTED'` **ДО** ack 200 (ключ дедупа = top-level ref, если MC
  шлёт его вне шифртекста, иначе `enc:sha256(ciphertext)`); сбой записи → **500**, чтобы MC
  ретраил (иначе событие потеряно навсегда). Декрипт протягивается позже (MTF/Prod — в
  sandbox «Not Applicable»; + per-tenant seam).
