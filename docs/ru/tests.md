...
# Юнит-наборы: 28 наборов / 202 теста (rootDir src, *.spec.ts)
node node_modules\jest\bin\jest.js

# Герметичный E2E (MC застаблен, без сети): 18 тестов — npm `test:e2e`
#   test/app.contract.e2e-spec.ts через ./test/jest-e2e.json
node node_modules\jest\bin\jest.js --config ./test/jest-e2e.json

# Живой E2E против ЖИВОГО sandbox: 23 теста — npm `test:e2e:live`
#   test/app.e2e-spec.ts через ./test/jest-e2e-live.json
...
> как в `src/harness/main.ts` / `src/harness/app.module.ts`) и гоняет его через `axios`.
...
> Путь дедупа по `eventRef` теперь персистится в `tx_status` (атомарный
> `INSERT … ON CONFLICT` — без слоя `kv_store`) и зафиксирован юнит-тестами
