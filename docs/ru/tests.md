...
| # | Тест (заголовок `it`) | Факт |
|---|---|---|
| MC-10 | `GET /crossborder/rfi/requests/:id` → проброс бизнес-`400` | ✅ `400` + тело MC содержит `062000` |
| MC-11 | `POST /crossborder/rfi/requests/:id` (update) | ✅ доходит до MC (не 404/500) |
| MC-12 | `POST /crossborder/rfi/documents` (upload) | ✅ доходит до MC (не 404/500) |
| MC-13 | `GET /crossborder/rfi/documents/:id` (download magic-id) | ✅ доходит до MC (не 500) |
| MC-14 | `POST /crossborder/rfi/documents` с файлом ~500KB | ✅ **НЕ 413** — route-scoped лимит 2MB, а не глобальный 256kb |

> **Корень RFI-ошибок (разобрано 2026-06-16, эмпирически).** `request_id` ДОЛЖЕН быть
> **валидным UUID по RFC-4122**. Наши демо-id (`33000000-0000-0000-0000-000000000000`,
> `10000000-…-082000`) намеренно невалидны — ниблы версии/варианта = 0, поэтому MC отвечает
> `400` `062000 INVALID_INPUT_FORMAT "Value contains invalid character"` (Source: `request_id`);
> то же с буквальным плейсхолдером доки `33XXXXXX-…` (X — невалидные hex-символы). С **валидной
> v4-формой** (`33000000-0000-4000-8000-000000000000`) MC проходит валидацию формата, но в
> sandbox возвращает **`401`** → шлюз маскирует в `502`: наш `partner-id` `SANDBOX_1234567`
> **не онбординжен для RFI** (внешний лимит sandbox, как Endpoint Guide; не баг шлюза). Upload
> документов тоже `401`→`502` по той же причине. MC-10 проверяет контракт object-4xx passthrough.
