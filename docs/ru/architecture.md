...
- **R6.** Секреты (ключи, пароли, consumer keys) — в **Secret Manager (AWS Secrets Manager)**.
...
| секреты партнёров | **AWS Secrets Manager** (через `SecretStore`) |
...
| `secretRef` | имя/ARN секрета в AWS Secrets Manager (для `OWN`) |
...
4. `CredentialsService.resolve(tenant)` → `McCredentials` (AWS Secrets Manager, через кэш).
...
| `SecretsModule` | `SecretStore`: Local (dev) / AWS Secrets Manager (прод) |
...
| `WebhooksModule` | приём push-уведомлений MC (auth решается в приложении: валидация mTLS клиентского cert при `webhookMtlsEnabled`, иначе fail-closed `X-Webhook-Token`; ингресс — L4 passthrough, не орган безопасности); ВСЕ события персистятся/дедупятся в `tx_status` одним атомарным `INSERT ON CONFLICT` по `eventRef` (KV-слоя нет); статусные (STATUS_CHG/QUOTE_STATUS_CHG) несут status/stage и читаются мерчантом, атрибуция тенанту (OWN→partnerId / PLATFORM→общий пул), нормализация camel/snake |
...
- **Прод-гейты:** запрет старта со слабыми/дефолтными секретами (`isWeakSecret()`, общий
  для `main.ts` и `GatewayConfig`) и без `MC_SECRET_STORE=aws-secrets-manager`. helmet / лимит тела /
...
- AWS Secrets Manager: короткоживущий кэш, ротация ключей без рестарта (`invalidate`).
...
- ✅ **Фаза 2 — SecretStore (Local/AWS Secrets Manager) + режим OWN.**
...
- ⬜ **Перед прод (деплойное):** сильные секреты, mTLS для вебхуков MC, OWN-ключи в AWS Secrets
  Manager, `migration:run`, MTF-подтверждение (кросс-тенант FLE + зашифрованный push);
  опционально метрики/трейсинг (Prometheus/OTel) — см. [production-questions.md](./production-questions.md).
