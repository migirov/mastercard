...
3. **Увидеть РЕАЛЬНЫЙ вызов Mastercard** 👇
   ...
   - Нажми **«Validate»** рядом с IBAN и рядом с адресом → появится зелёный бейдж
     **«Validated · Mastercard»**. **Это настоящий вызов песочницы Mastercard** через наш
     шлюз (`SUCCESS` / `VALID`+`VERIFIED`). Берите именно этот счёт — только его адрес
     песочница и подтверждает.
   - На любом другом счёте адрес покажет **«Not checked»** — это правильно, а не баг:
     песочница отклоняет все страны кроме `USA` и подтверждает лишь один адрес. Селектор
     **Address country** рядом с полем показывает, по какой стране шла проверка
     (предзаполнен по IBAN, редактируется). См. [docs/ru/test.md §1.1](docs/ru/test.md).
...
| `validate-address` | 🟡 **живой вызов, фикстура** | Песочница подтверждает **один** документированный адрес и принимает только `country: USA` — всё остальное 400, показывается как «Not checked», а не «невалидно» ([§1.1](docs/ru/test.md)) |
