...
ключи подписи и шифрования) резолвятся на каждый запрос, но эндпоинт и схема
авторизации — общие на процесс.

Клиентский сертификат исходящего mTLS сейчас **только платформенный**: в секрет-бандле
мерчанта нет поля под него, так что свой сертификат тенант принести не может. Поэтому
там, где Mastercard требует mTLS, OWN-тенанту без своего сертификата мы **отказываем**,
а не проводим его под платформенной TLS-личностью — иначе Mastercard увидел бы, что
сертификат одной организации аутентифицирует partner-аккаунт другой.
