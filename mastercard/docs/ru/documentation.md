...
**McWebhookEvent** — payload push-уведомления Mastercard (`POST /webhooks/mastercard/webhook`).
...
- **Аутентификация:** решается **в приложении**, не на ингрессе. MC аутентифицирует push
  только **клиентским сертификатом** (mTLS) — ни токена, ни заголовка, ни подписи (JWS/HMAC у
  MC нет; бывший «C1» закрыт чтением доки). При `webhookMtlsEnabled` `WebhookAuthGuard`
  валидирует cert MC в приложении: доверенная цепочка (`socket.authorized`) + allowlist по
  subject-CN. Fail-closed `X-Webhook-Token` — опциональный dev/вторичный фактор (когда in-app
  TLS выключен). Детали и цитата MC — `api.md` → Webhooks.
